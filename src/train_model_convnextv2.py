"""
Part 3b: Train F+I Model — ConvNeXt V2 encoder

Same F+I architecture but replaces Inception v3 with ConvNeXt V2 (Base).
ConvNeXt V2 uses a self-supervised FCMAE pretraining + supervised fine-tuning,
producing 1024-dim embeddings (vs Inception's 2048).

  Image branch:   ConvNeXt V2 (frozen) → 1024 → FC512 → FC64
  Feature branch: tabular features → FC64
  Fusion:         concat(64+64) → FC → price prediction

Reads:
  - data/processed/merged_<year>.geojson
  - data/images/<year>/<ssl>.png

Produces:
  - data/embeddings/image_embeddings_convnextv2_<year>.npy
  - data/embeddings/ssl_order_convnextv2_<year>.npy
  - models/model_fi_convnextv2_<year>.pt
  - models/results_fi_convnextv2_<year>.json
"""
...
EMBEDDING_DIM = 1024  # ConvNeXt V2 Base output dimension
...
def paths_for_year(year: int):
    return {
        ...
        "image":   DATA_DIR / "images" / str(year),
        "embed":   EMBED_DIR / f"image_embeddings_convnextv2_{year}.npy",
        "ssl":     EMBED_DIR / f"ssl_order_convnextv2_{year}.npy",
    }
...
def output_paths(years: list[int]):
    ...
    return {
        "model":   MODEL_DIR / f"model_fi_convnextv2_{tag}.pt",
        "results": MODEL_DIR / f"results_fi_convnextv2_{tag}.json",
    }
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 2: Extract ConvNeXt V2 embeddings from saved images
# ══════════════════════════════════════════════════════════════════════════════

def build_convnextv2_encoder():
    """Load ConvNeXt V2 Base with classifier removed → 1024-dim output."""
    local_weights = DATA_DIR / "pretrained_weights" / "convnext_base.pth"
    ...
    model.classifier = nn.Sequential(
        ...
        # drop the final Linear(1024, 1000) — keep 1024-dim features
    )
    model.eval()
    model.to(DEVICE)
    return model
...
def extract_embeddings(gdf, year: int):
    """Load saved images and extract 1024-dim ConvNeXt V2 embeddings."""
    p = paths_for_year(year)
    ...
    encoder = build_convnextv2_encoder()
    ...
    print(f"\nExtracting ConvNeXt V2 embeddings for {total:,} lots from {image_dir} …")
    with torch.no_grad():
    ...
    print(f"Saved {len(embeddings)} embeddings ({EMBEDDING_DIM}-dim) → {p['embed']}")
    if skipped:
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4: F+I Neural Network (adapted for 1024-dim ConvNeXt V2 embeddings)
# ══════════════════════════════════════════════════════════════════════════════

class HousePriceFI(nn.Module):
    """
    F+I model adapted for ConvNeXt V2:
      Image:   1024 → 512 → 64  (with dropout)
      Tabular: n_features → 64  (with dropout)
      Fusion:  128 → 1
    """
    def __init__(self, n_tabular_features: int, embedding_dim: int = EMBEDDING_DIM):
        super().__init__()
...
def train_model(embeddings, X_tabular, y_log, y_raw, scaler_y, years: list[int],
    ...
    print("\nTraining F+I model (ConvNeXt V2) …")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train F+I model (ConvNeXt V2)")
    ...
    all_embeddings, all_ssl, all_gdf = [], [], []
    for year in years:
        gdf = load_data(year)
    ...
    all_X, all_y_norm, all_y_raw = [], [], []
    for gdf, ssl_order in all_gdf:
        X, yn, yr, _, _ = prepare_tabular(gdf, ssl_order)
        all_X.append(X)
